...
| `--kne-modal-body-min-height` / `--kne-modal-body-max-height` | body 容器 min/max。min 随 `size`：small 基准 300、default/large 基准 500，再扣 title/footer chrome（有 footer 时约再减 58）；CSS 回退 small `252px` / 其它 `394px`。max 等于 `body-height`。桌面默认滚动：`height:auto`，不低于 min、不超过 max |
